    ...
    private static void Main(string[] args)
    {
        ...
        try
        {
            producer = new ProducerBuilder<string, string>(producerConfig)
                // Перехватываем внутренние системные ошибки rdkafka
                .SetErrorHandler((p, error) => 
                {
                    Console.WriteLine($"[KAFKA CRITICAL ERROR] Код: {error.Code}, Причина: {error.Reason}");
                    if (error.IsFatal)
                    {
                        // Если ошибка фатальная, логируем и завершаем работу приложения, так как дальнейшая работа без Kafka может быть бессмысленной для данного скрипта. Это предотвращает бесконечные попытки отправки сообщений в случае серьезных проблем с брокером.
                        Console.WriteLine("Фатальная ошибка Kafka. Завершение работы скрипта.");
                        Environment.Exit(1);
                    }
                })
                .Build();

            // Тестовая проверка связи с Kafka перед началом мониторинга. Если брокер недоступен, скрипт завершится с информативным сообщением, не запуская основной цикл. 
